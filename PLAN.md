...
- ✅ **Memória de cliente** (auto-cadastro pelo WhatsApp + ficha no prompt: "Boa tarde, Carlos!"), **endereço obrigatório no delivery**, **comanda térmica 80mm** (botão + impressão automática por aparelho via iframe), **badge de pendentes no menu**, **anti-duplicata** (15 min) e **pedido automático** (toggle por número → encomenda oficial na hora). Migration `20260706000002` **APLICADA**.
- ✅ **Pacote de operação (as 6 melhorias)**: informações oficiais por loja editáveis na aba Robô (robô responde horário/endereço/pagamento/taxa), template aprovado no aviso (fura janela 24h), **agente de impressão local** (`scripts/agente-impressao.mjs` + API `/api/atendimento/comandas` com `IMPRESSAO_TOKEN`), som de pedido novo por aparelho, **aba Relatório** (mês/canal/conversão/top produtos) e cadastro do cliente pela conversa. Migration `20260706000003` **APLICADA**.
- ✅ **Blindagem pré-go-live**: assinatura do webhook (`META_APP_SECRET`, 401 pra não-Meta — validado vivo), anti-abuso (6/min, 40/h), **pedido multi-itens** (JSONB + item por produto no virar pedido/auto), fora do expediente (dia/hora no prompt), alerta de pane no WhatsApp do admin (`ALERTA_WHATSAPP`), Imprimir/PDF no Relatório, notificação do navegador e card do robô no Resumo. Migration `20260707000000` **APLICADA**. Commit `3f59f98` pushed. 105 testes ✅.
- 🔲 **Envs opcionais no go-live**: `META_APP_SECRET` (App Secret do app na Meta — fecha o webhook) e `ALERTA_WHATSAPP` (número do Nicholas para alertas de pane).
- ✅ **Valores por nível na tela Produtos** (`c59dd82`): custo/preço/margem só p/ admin de produtos ou quem tem precos/painel; operação (escrita) usa os controles do robô sem ver números — corte no servidor (lista, drawer, modal, impressão). Validado E2E nos dois níveis.
- ✅ **Carta de passagem do SAC Agrindus** escrita em `docs/carta-sac-agrindus.md` — mapa pra portar o motor do robô (o que copiar de `lib/atendimento/`, o que virar `#CHAMADO#`, armadilhas e envs). Próximo case do agente.
