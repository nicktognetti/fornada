...
export function ConfigPanel({ dbCategorias }: Props) {
  const [activeTab, setActiveTab] = useState<TabId>('tipos')

  // ── Estado de cada seção ───────────────────────────────────────────────────
  const [tipos, setTipos] = useState<string[]>(SEED_TIPOS)

  const [unidades, setUnidades] = useState<{ label: string; nome: string }[]>(SEED_UNIDADES)
  const [novaLabel, setNovaLabel] = useState('')
  const [novaNome, setNovaNome] = useState('')

  const [categorias, setCategorias] = useState<string[]>(() => {
    const merged = [...new Set([...SEED_CATEGORIAS, ...dbCategorias])]
    return merged.sort()
  })
  ...
  function removeTipo(v: string) {
    setTipos((t) => t.filter((x) => x !== v))
  }
  ...
  function removeUnidade(label: string) {
    setUnidades((u) => u.filter((x) => x.label !== label))
  }
  ...
  function removeCategoria(v: string) {
    setCategorias((c) => c.filter((x) => x !== v))
  }
  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      <div
        className="flex items-start gap-3 rounded-xl px-4 py-3 border border-blue-500/20 bg-blue-500/8"
      >
      ...
      <div
        ...
        style={{ backgroundColor: 'var(--t-input-bg)', border: '1px solid var(--t-input-border)' }}
      >
        {tabs.map((tab) => (
          <button
            key={tab.id}
      ...
      {activeTab === 'tipos' && (
        ...
        >
          <div className="px-5 py-4" style={{ borderBottom: '1px solid var(--t-border-sub)', backgroundColor: 'var(--t-inset-bg)' }}>
            <p className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: 'var(--t-accent)' }}>
              Tipos de Receita
            </p>
            <p className="text-xs mt-0.5" style={{ color: 'var(--t-text-2)' }}>
              Classificação usada ao criar fichas técnicas.
            </p>
          </div>
          ...
          <div
            ...
            style={{ borderTop: '1px solid var(--t-border-sub)' }}
          >
            <input
              ref={tipoInputRef}
              type="text"
              ...
              placeholder="Novo tipo… (ex: Glacê)"
              className="input-field flex-1"
            />
            <button onClick={addTipo} className="btn-primary px-4 min-h-[40px]" style={{ minHeight: 40 }}>
              <Plus size={15} />
              Adicionar
            </button>
          </div>
      ...
      {activeTab === 'unidades' && (
        ...
        >
          <div className="px-5 py-4" style={{ borderBottom: '1px solid var(--t-border-sub)', backgroundColor: 'var(--t-inset-bg)' }}>
            ...
            <p className="text-xs mt-0.5" style={{ color: 'var(--t-text-2)' }}>
              Unidades disponíveis nos seletores de insumos e receitas.
            </p>
          </div>
          ...
          <div className="divide-y" style={{ borderColor: 'var(--t-border-sub)' }}>
            {unidades.map((u) => (
              <div key={u.label} className="flex items-center gap-3 px-5 py-3">
                <span className="font-mono font-bold text-sm w-10 shrink-0" style={{ color: 'var(--t-accent)' }}>
                  {u.label}
                </span>
                <span className="text-sm flex-1" style={{ color: 'var(--t-text-2)' }}>
                  {u.nome}
                </span>
                <button
                  onClick={() => removeUnidade(u.label)}
          ...
          >
            ...
            <input
              ...
              onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addUnidade())}
              placeholder="Nome completo (ex: dúzia)"
              className="input-field flex-1"
            />
            <button onClick={addUnidade} className="btn-primary px-4 shrink-0" style={{ minHeight: 40 }}>
              <Plus size={15} />
            </button>
          </div>
        </div>
      ...
      {activeTab === 'categorias' && (
        ...
        >
          <div className="px-5 py-4" style={{ borderBottom: '1px solid var(--t-border-sub)', backgroundColor: 'var(--t-inset-bg)' }}>
            ...
            <p className="text-xs mt-0.5" style={{ color: 'var(--t-text-2)' }}>
              Agrupamentos para filtrar e organizar os insumos.
            </p>
          </div>
          ...
          >
            <input
              ...
              placeholder="Nova categoria… (ex: Sementes)"
              className="input-field flex-1"
            />
            <button onClick={addCategoria} className="btn-primary px-4 shrink-0" style={{ minHeight: 40 }}>
              <Plus size={15} />
              Adicionar
            </button>
          </div>
      ...
      {/* Contagem */}
      <p className="text-xs text-right" style={{ color: 'var(--t-text-2)', opacity: 0.5 }}>
