...
export function FichaView({ receita, custo, itens }: Props) {
  ...
  function openEditReceita() {
    setEditReceitaKey(Date.now())
    setEditReceitaOpen(true)
  }

  function openAddItem() {
    setItemModal({ open: true, item: null, key: Date.now() })
  }

  function openEditItem(item: ReceitaItemComCusto) {
    setItemModal({ open: true, item, key: Date.now() })
  }
  ...
  return (
    <>
      ...
      <div className="card-surface px-6 py-5 mb-4">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <h1 className="font-playfair text-creme text-[28px] sm:text-[34px] font-bold leading-tight">
              {receita.nome}
            </h1>
            {receita.observacao && (
          ...
          {custo?.custo_total != null && custo.custo_total > 0 && (
            <div className="shrink-0 text-right">
              ...
              <p className="font-playfair text-croissant text-[30px] sm:text-[36px] font-bold leading-none">
        ...
        <div className="flex gap-2 mt-5 pt-4 border-t border-white/[0.06]">
          ...
          {!confirmDelete ? (
            <button
              ...
              className="btn-ghost text-xs px-4 py-2 min-h-[36px] border-red-400/20 text-red-400/70 hover:text-red-400 hover:border-red-400/40"
            >
              <Trash2 size={13} />
              Excluir
            </button>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={handleDeleteReceita} className="text-xs text-red-400 border border-red-400/30 hover:bg-red-400/10 rounded-lg px-3 py-1.5 transition-colors">
                Sim, excluir
              </button>
              <button onClick={() => setConfirmDelete(false)} className="text-xs text-demerara hover:text-creme px-2 py-1.5">
                Cancelar
              </button>
            </div>
          )}
      ...
      {/* Alerta de pendentes */}
      {pendentes.length > 0 && (
        <div className="flex items-start gap-3 rounded-xl border border-amber-400/25 bg-amber-400/8 px-4 py-3 mb-4">
      ...
      <div className="card-surface overflow-hidden mb-4">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-white/[0.06]">
          <div className="flex items-center gap-2">
            <BookOpen size={15} className="text-croissant" />
            <span className="text-croissant text-[11px] uppercase tracking-widest font-semibold">
              Ingredientes
            </span>
            <span className="text-demerara/50 text-xs">({itens.length})</span>
          </div>
          ...
          <div>
            ...
            <div className="hidden md:block">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[#0f0f14] text-demerara text-[11px] uppercase tracking-wider">
                    <th className="text-left px-5 py-2.5 font-medium">Ingrediente</th>
                    <th className="text-right px-4 py-2.5 font-medium">Qtd</th>
                    <th className="text-left px-2 py-2.5 font-medium w-12">Un</th>
                    <th className="text-right px-4 py-2.5 font-medium">Custo/un</th>
                    <th className="text-right px-5 py-2.5 font-medium">Custo item</th>
                    <th className="px-4 py-2.5 w-20" />
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/[0.05]">
                  {[...pendentes, ...normais].map(item => (
                    <tr key={item.id} className={`transition-colors ${item.is_pendente ? 'bg-amber-400/5' : 'hover:bg-white/[0.02]'}`}>
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          {item.is_pendente && <AlertTriangle size={13} className="text-amber-400 shrink-0" />}
                          <span className={`font-medium ${item.is_pendente ? 'text-amber-400' : 'text-creme'}`}>
                            {item.nome_display}
                          </span>
                          {item.sub_receita_id && !item.is_pendente && (
                            <span className="text-[10px] text-croissant/70 border border-croissant/20 rounded px-1.5 py-0.5">sub</span>
                          )}
                        </div>
                      </td>
                      <td className="px-4 py-3 text-right text-creme">{item.quantidade}</td>
                      ...
                      <td className="px-4 py-3 text-right text-demerara text-xs">
                        {item.custo_unitario != null
                          ? formatCustoUso(item.custo_unitario, item.unidade)
                          : '—'}
                      </td>
                      <td className="px-5 py-3 text-right font-playfair font-semibold text-[15px] text-creme">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <button
                            ...
                            className="w-7 h-7 rounded-lg flex items-center justify-center text-demerara/40 hover:text-creme hover:bg-white/8 transition-all"
                            aria-label="Editar"
                          >
                            <Pencil size={12} />
                          </button>
                          <button
                            ...
                            className="w-7 h-7 rounded-lg flex items-center justify-center text-demerara/40 hover:text-red-400 hover:bg-red-400/10 transition-all"
                            aria-label="Remover"
                          >
                            <Trash2 size={12} />
                          </button>
            ...
            <div className="md:hidden divide-y divide-white/[0.05]">
              {[...pendentes, ...normais].map(item => (
                <div key={item.id} className={`px-4 py-3 ${item.is_pendente ? 'bg-amber-400/5' : ''}`}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        {item.is_pendente && <AlertTriangle size={13} className="text-amber-400 shrink-0" />}
                        <p className={`text-sm font-medium truncate ${item.is_pendente ? 'text-amber-400' : 'text-creme'}`}>
                          {item.nome_display}
                        </p>
                        {item.sub_receita_id && !item.is_pendente && (
                          <span className="text-[10px] text-croissant/70 border border-croissant/20 rounded px-1 py-0.5 shrink-0">sub</span>
                        )}
                      </div>
                      <p className="text-demerara text-xs mt-0.5">
                        {item.quantidade} {item.unidade}
                        {item.custo_item != null && (
                          <span className="ml-2 text-creme font-playfair text-sm">R$ {formatBRL(item.custo_item)}</span>
                        )}
                      </p>
                    </div>
                    <div className="flex gap-1 shrink-0">
                      <button onClick={() => openEditItem(item)} className="w-8 h-8 rounded-lg flex items-center justify-center text-demerara/50 hover:text-creme hover:bg-white/8 transition-all">
                        <Pencil size={13} />
                      </button>
                      <button onClick={() => handleRemoveItem(item)} disabled={deletingItemId === item.id} className="w-8 h-8 rounded-lg flex items-center justify-center text-demerara/50 hover:text-red-400 hover:bg-red-400/10 transition-all">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </div>
            ...
            {/* Rodapé com total */}
            {custo?.custo_total != null && custo.custo_total > 0 && (
              <div className="flex items-center justify-between px-5 py-4 border-t border-white/[0.06] bg-[#0f0f14]">
      ...
      {editReceitaOpen && (
        <ReceitaModal
          key={editReceitaKey}
          receita={receita}
          onClose={() => setEditReceitaOpen(false)}
        />
      )}
